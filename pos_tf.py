...
#                            memory_key_padding_mask=memory_key_padding_mask)
        output = self.norm(output)
        # print('decoder ff : ', torch.max(output, dim=-1)[1])
        return output
...
#                 memory_key_padding_mask=None):
        ...
        # print('decoder ca : ', torch.max(x, dim=-1)[1])
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask,): #attn_mask=None, key_padding_mask=None, memory=False):
        ...
        #     print(f'ca mask : {mask}, shape : {mask.shape}')
        # else:
        #     print(f'enc or dec mask : {mask}, shape : {mask.shape}')
        # [batch size, n_heads, src time stpes, head dim]
        Q = Q.view(batch_size, -1, self.n_heads, self.head_dim).permute(0, 2, 1, 3)
        K = K.view(batch_size, -1, self.n_heads, self.head_dim).permute(0, 2, 1, 3)
        V = V.view(batch_size, -1, self.n_heads, self.head_dim).permute(0, 2, 1, 3)
        # print('Q : ', Q.size(), 'K : ', K.size(), 'Q type : ', Q.dtype, 'K type : ', K.dtype) 
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, token_embedding, init=0): # (B, T, d_model)
    # 배치만큼 브로드 캐스팅이 일어나서 위치 정보를 더해줄 것
        return self.dropout(token_embedding + self.pos_embedding[:, init:init+token_embedding.size(1), :].to(token_embedding.device))
...
#                                   memory_key_padding_mask = src_padding_mask)
    ...
    def search(self, src, src_mask, max_length=20, bos_id=2, eos_id=3):
        ...
        with torch.no_grad():
            ...
            # src_emb = (self.src_tok_emb(src) / self.scale) + self.src_pos_emb(src_pos).squeeze(0)
            # print((self.src_tok_emb(src) / self.scale).shape, self.src_pos_emb(src_pos).squeeze(0).shape)
            ######### batch size 1 #######################
            # print(src_emb.shape)
            ...
            for t in range(max_length):
                pos_input = self.positional_encoding(self.tgt_tok_emb(dec_input), init=t)
                # tgt_pos = torch.arange(0, dec_input_len.item()).unsqueeze(0).repeat(batch_size, 1)
                # dec_input = (self.tgt_tok_emb(dec_input) / self.scale) + self.tgt_pos_emb(torch.LongTensor(tgt_pos)).squeeze(0)
                ...
                ## rev.
                logits = self.transformer.decoder(pos_input,
                                                  memory,
                                                  src_mask=src_mask,
                                                  tgt_mask=None)
                                                #  tgt_key_padding_mask=None,
                ...
                # print(dec_input.shape, next_item.shape)
